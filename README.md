# 📚 Great Cheatsheets

- A growing collection of cheatsheets for programming languages, tools, and frameworks — built as I learn and expand my skills.

---

## 🗂️ Contents

| File | Language / Tool | Format | Description |
|------|----------------|--------|-------------|
| [python-intro.py](./python-intro.py) | 🐍 Python | `.py` | Runnable Python code examples |
| [python-intro.md](./python-intro.md) | 🐍 Python | `.md` | Python reference with tables & tips |
| [html-ref.md](./html-ref.md) | 🌐 HTML | `.md` | HTML tags & elements reference |
| [git.sh](./git.sh) | 🔧 Git | `.sh` | Git commands as shell comments |
| [git.md](./git.md) | 🔧 Git | `.md` | Git reference with tables & workflow |

---

## 🐍 Python — `python-intro.py` & `python-intro.md`

Covers the core building blocks of Python:

- Variables & Data Types
- Output & Input (`print`, `input`)
- Arithmetic, Comparison & Logical Operators
- Strings, Indexing & Slicing
- Lists & List Methods
- Dictionaries
- Conditionals (`if`, `elif`, `else`)
- Loops (`for`, `while`, `break`, `continue`)
- Functions (default params, `*args`, multiple returns)
- Imports & the `math` module

> The `.py` file contains runnable code. The `.md` file is formatted for easy reading on GitHub.

---

## 🌐 HTML — `html-ref.md`

A complete markdown reference guide covering:

- Page Structure & HTML5 Boilerplate
- Text, Headings & Blockquotes
- Links & Navigation
- Images, Video, Audio & Iframes
- Lists (unordered, ordered, nested, description)
- Tables (`thead`, `tbody`, `colspan`, `rowspan`)
- Forms & all Input Types
- Semantic Elements (`header`, `main`, `article`, `section`, `footer`)
- Div, Span & Grouping
- Meta & Head Tags (SEO, fonts, scripts)
- Inline Text Formatting
- Special Characters & Miscellaneous

---

## 🔧 Git — `git.sh` & `git.md`

A comprehensive reference for everyday Git usage:

- Configuration & Setup
- Starting a Repository (`init`, `clone`)
- Staging & Committing
- Viewing History & Diffs
- Undoing Changes (`restore`, `revert`, `reset`)
- Branches (create, switch, rename, delete)
- Merging & Rebasing
- Remote Repositories (`fetch`, `pull`, `push`)
- Stashing
- Tags
- Aliases & Shortcuts
- `.gitignore` Patterns
- Everyday Workflow & Commit Message Conventions

> The `.sh` file uses shell comment syntax. The `.md` file is formatted for easy reading on GitHub.

---

## 🚀 Roadmap

Cheatsheets planned for future addition:

- [ ] CSS
- [ ] JavaScript
- [ ] SQL
- [ ] Bash / Linux
- [ ] React
- [ ] TypeScript
- [ ] Markdown
- [ ] Docker

---

## 👤 About

This repository is a personal learning resource. As I pick up new languages and tools, I add a cheatsheet to keep everything in one place for quick reference.

---

> *"The more that you read, the more things you will know."* — Dr. Seuss