# Great Cheatsheets

## Learning Progress
- **Completed**: 2 sections
- **In Progress**: 1 pillar  
- **Not Started**: 4 pillars
- **Last updated**: 2026-05-13

---

## Pillar 1 — Networking (IN PROGRESS) [2/7]
*Completed: 2026-05-13*

- [x] **Section 1 — How the internet is structured** ✓
  - OSI model, layers, protocols overview
  - [View notes](./01.%20How%20the%20Internet%20is%20Structured)

- [x] **Section 2 — IP addressing** ✓
  - IPv4/IPv6, address formats, public/private ranges
  - [View notes](./05.%20IP-Addressing.md)

- [ ] **Section 3 — Subnetting and CIDR** ← next
  - Subnet masks, CIDR notation, subnet calculations
  - [View notes](./Section3-Subnetting-CIDR.md)

- [ ] **Section 4 — Routing and switching**
  - Routing protocols, switching concepts, network paths
  - [View notes](./Section4-Routing-Switching.md)

- [ ] **Section 5 — DNS**
  - Domain name resolution, DNS records, DNS servers
  - [View notes](./Section5-DNS.md)

- [ ] **Section 6 — TCP and UDP**
  - Transport layer protocols, ports, connection management
  - [View notes](./Section6-TCP-UDP.md)

- [ ] **Section 7 — Network security basics**
  - Firewalls, encryption, common attacks, best practices
  - [View notes](./Section7-Network-Security.md)

---

<details>
<summary><b>Pillar 2 — Linux & CLI (not started)</b></summary>

- [ ] Section 1 — Basic commands
- [ ] Section 2 — File systems
- [ ] Section 3 — Users and permissions
- [ ] Section 4 — Process management
- [ ] Section 5 — Package managers

</details>

<details>
<summary><b>Pillar 3 — Security (not started)</b></summary>

- [ ] Section 1 — Cryptography basics
- [ ] Section 2 — Authentication & authorization
- [ ] Section 3 — Common vulnerabilities
- [ ] Section 4 — Secure coding practices
- [ ] Section 5 — Penetration testing basics

</details>

<details>
<summary><b>Pillar 4 — Scripting (not started)</b></summary>

- [ ] Section 1 — Shell scripting fundamentals
- [ ] Section 2 — Python basics
- [ ] Section 3 — Automation scripts
- [ ] Section 4 — Error handling
- [ ] Section 5 — Best practices

</details>

<details>
<summary><b>Pillar 5 — Databases (not started)</b></summary>

- [ ] Section 1 — SQL fundamentals
- [ ] Section 2 — Database design
- [ ] Section 3 — Indexing & optimization
- [ ] Section 4 — Transactions & ACID
- [ ] Section 5 — NoSQL basics

</details>
